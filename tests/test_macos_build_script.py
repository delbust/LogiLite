...
class MacOSBuildScriptTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / "build_resources" / "LogiLite.entitlements").write_text(
            "<plist/>", encoding="utf-8"
        )
        (self.root / "LogiLite-mac.spec").write_text("# fake spec", encoding="utf-8")
    ...
    def _write_codesign(self):
        self._write_command(
            ...
            textwrap.dedent(
                ...
                case "$*" in
                  *"--verify"*)
                    if [ "${LOGILITE_CODESIGN_VERIFY_FAIL:-0}" = "1" ]; then
                      exit 31
                    fi
                    ;;
                esac
    ...
    def _write_python(self, path: Path, label: str):
        self._write_executable(
            ...
            textwrap.dedent(
                f"""
                label={label!r}
                printf 'python\\t%s\\t%s\\t%s\\n' "$label" "${{PYTHONHASHSEED:-}}" "$*" >> "$LOGILITE_TEST_LOG"
                if [ "${{1:-}}" = "-c" ]; then
                  case "${{2:-}}" in
                    *"import PyInstaller; print"*)
                      ...
                      ;;
                    *"import PyInstaller"*)
                      if [ "${{LOGILITE_FAKE_NO_PYINSTALLER:-}}" = "$label" ]; then
                        exit 13
                      fi
                      exit 0
                      ;;
                ...
                if [ "${{1:-}}" = "-m" ] && [ "${{2:-}}" = "PyInstaller" ]; then
                  ...
                  app="$LOGILITE_TEST_ROOT/dist/LogiLite.app"
    ...
    def _run_script(self, restricted_path=False, **overrides):
        ...
        env["LOGILITE_TEST_LOG"] = str(self.log)
        env["LOGILITE_TEST_ROOT"] = str(self.root)
        for key in (
            "LOGILITE_PYTHON",
            "LOGILITE_SIGN_IDENTITY",
            "LOGILITE_FAKE_NO_PYINSTALLER",
            "LOGILITE_CODESIGN_VERIFY_FAIL",
            "LOGILITE_PREFER_PYENV",
    ...
    def test_logilite_python_wins_and_bad_override_fails(self):
        custom = self.root / "custom-python"
        self._write_python(custom, "logilite-python")
        (self.root / ".venv" / "bin").mkdir(parents=True)
        self._write_python(self.root / ".venv" / "bin" / "python3", "repo-python3")

        result = self._run_script(LOGILITE_PYTHON=custom)

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("source: LOGILITE_PYTHON", result.stdout)
        self.assertEqual(self._pyinstaller_labels(), ["logilite-python"])

        self.log.unlink()
        result = self._run_script(LOGILITE_PYTHON=self.root / "missing-python")

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("LOGILITE_PYTHON is set but is not executable", result.stderr)
    ...
    def test_missing_pyinstaller_names_selected_interpreter(self):
        ...
        result = self._run_script(LOGILITE_FAKE_NO_PYINSTALLER="repo-python3")
    ...
    def test_identity_signing_order_and_verify_failure(self):
        ...
        self.log.unlink()
        result = self._run_script(
            LOGILITE_SIGN_IDENTITY="IDENTITY",
            LOGILITE_CODESIGN_VERIFY_FAIL="1",
        )
