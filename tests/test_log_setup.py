...
class GetLogDirTests(unittest.TestCase):
    def test_darwin_returns_library_logs_logilite(self):
        with patch.object(sys, "platform", "darwin"):
            result = log_setup._get_log_dir()
        self.assertTrue(result.endswith(os.path.join("Library", "Logs", "LogiLite")))
    ...
    def test_linux_uses_xdg_state_home(self):
        with (
            ...
            result = log_setup._get_log_dir()
        self.assertEqual(result, os.path.join("/custom/state", "LogiLite", "logs"))
    ...
    def test_linux_defaults_to_dot_local_state(self):
        ...
        expected = os.path.join(
            os.path.expanduser("~"), ".local", "state", "LogiLite", "logs"
        )
    ...
    def test_windows_uses_appdata(self):
        ...
        with (
            ...
            result = log_setup._get_log_dir()
        self.assertEqual(result, os.path.join(fake_appdata, "LogiLite", "logs"))
...
class SetupLoggingTests(unittest.TestCase):
    ...
    def test_creates_log_file_on_startup(self):
        ...
        self.assertEqual(path, os.path.join(self.tmp, "logilite.log"))
    ...
    def test_print_output_written_to_log_file(self):
        ...
        for h in logging.root.handlers:
            h.flush()
        log_path = os.path.join(self.tmp, "logilite.log")
