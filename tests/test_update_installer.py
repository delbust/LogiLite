...
def _payload(**updates):
    ...
    payload = {
        ...
        "commit": "abc123",
        "release_notes_url": "https://github.com/delbust/LogiLite/releases/tag/v3.7.0",
        "assets": {
            "windows-x64": {
                "name": "LogiLite-Windows.zip",
                "url": "https://github.com/delbust/LogiLite/releases/download/v3.7.0/LogiLite-Windows.zip",
                "size": 123,
                "sha256": "a" * 64,
            },
            "macos-arm64": {
                "name": "LogiLite-macOS.zip",
                "url": "https://github.com/delbust/LogiLite/releases/download/v3.7.0/LogiLite-macOS.zip",
                "size": 456,
                "sha256": "b" * 64,
            },
        },
...
class UpdateInstallerTests(unittest.TestCase):
    ...
    def test_manifest_name_matches_release_url_contract(self):
        self.assertEqual(manifest_name_for_version("v3.7.0"), "logilite-v3.7.0-update.json")
        self.assertTrue(
            manifest_url_for_release("v3.7.0").endswith(
                "/v3.7.0/logilite-v3.7.0-update.json"
            )
        )
    ...
    def test_manifest_url_can_use_test_endpoint_override(self):
        with patch.dict(
            "os.environ",
            {"LOGILITE_UPDATE_MANIFEST_URL": "http://127.0.0.1:8765/update.json"},
        ):
            self.assertEqual(
    ...
    def test_fetch_json_accepts_utf8_bom_response(self):
        payload = b'\xef\xbb\xbf{"schema": 1, "app_id": "io.github.delbust.logilite"}'
    ...
    def test_update_manifest_verifies_selected_platform(self):
        ...
        self.assertEqual(manifest.assets["windows-x64"].name, "LogiLite-Windows.zip")
    ...
    def test_archive_validator_accepts_windows_bundle_shape(self):
        with tempfile.TemporaryDirectory() as tmp:
            archive = Path(tmp) / "LogiLite-Windows.zip"
            _zip(
                archive,
                [
                    ("LogiLite/LogiLite.exe", b"exe"),
                    ("LogiLite/_internal/runtime.dll", b"dll"),
                ],
            )
            ...
            self.assertEqual(root, "LogiLite")
    ...
    def test_archive_validator_rejects_traversal_and_duplicates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _zip(duplicate, [("LogiLite/a.txt", b"1"), ("logilite/A.txt", b"2")])
            with self.assertRaises(UpdateInstallError) as ctx:
    ...
    def test_archive_validator_rejects_unsafe_path_forms(self):
        ...
        with self.assertRaises(UpdateInstallError) as ctx:
            _normalized_member_name("LogiLite/\x00evil.txt")
    ...
    def test_archive_validator_rejects_symlink_entries(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            info = zipfile.ZipInfo("LogiLite/link")
    ...
    def test_archive_validator_rejects_bad_crc(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _zip(archive, [("LogiLite/file.txt", b"CORRUPT_ME")])
    ...
    def test_archive_validator_rejects_empty_and_too_large_archives(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _zip(large, [("LogiLite/file.bin", b"123456")])
            with self.assertRaises(UpdateInstallError) as ctx:
    ...
    def test_archive_validator_rejects_incomplete_windows_bundle_shape(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _zip(no_exe, [("LogiLite/_internal/runtime.dll", b"dll")])
            with self.assertRaises(UpdateInstallError) as ctx:
                validate_zip_archive(
                    no_exe,
                    requirements=ArchiveRequirements(require_windows_app=True),
            ...
            _zip(no_runtime, [("LogiLite/LogiLite.exe", b"exe")])
            with self.assertRaises(UpdateInstallError) as ctx:
                validate_zip_archive(
                    no_runtime,
                    requirements=ArchiveRequirements(require_windows_app=True),
    ...
    def test_extract_validated_zip_writes_only_inside_stage_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((staged.app_root / "LogiLite.exe").exists())
    ...
    def test_extract_validated_zip_rejects_member_size_mismatch(self):
        class _FakeInfo:
            filename = "LogiLite/LogiLite.exe"
            file_size = 1
            external_attr = 0
        ...
        class _FakeRuntimeInfo:
            filename = "LogiLite/_internal/runtime.dll"
            file_size = 1
            external_attr = 0
    ...
    def test_platform_install_strategy_returns_manual_fallback_for_macos(self):
        manifest = UpdateManifest(
            ...
            assets={
                "macos-arm64": UpdateAsset(
                    "macos-arm64",
                    "LogiLite-macOS.zip",
                    "https://example.test/LogiLite-macOS.zip",
                    1,
                    "a" * 64,
                )
            },
        ...
        runtime = RuntimeLocation(
            executable=Path("/Applications/LogiLite.app/Contents/MacOS/LogiLite"),
            install_root=Path("/Applications/LogiLite.app"),
            app_data_dir=Path("/tmp/logilite"),
            frozen=True,
    ...
    def test_locate_runtime_classifies_source_windows_onedir_and_manual_platforms(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            install = root / "LogiLite Install With Spaces"
            install.mkdir()
            exe = install / "LogiLite.exe"
            ...
            mac_runtime = locate_runtime(
                executable=root / "LogiLite.app" / "Contents" / "MacOS" / "LogiLite",
                sys_platform="darwin",
                frozen=True,
                app_data_dir=root / "data",
            )
    ...
    def test_prepare_downloaded_asset_uses_default_download_timeout(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            target_path = root / "downloads" / "LogiLite-Windows.zip"
            target_path.parent.mkdir()
            target_path.write_bytes(b"verified")
            asset = UpdateAsset(
                "windows-x64",
                "LogiLite-Windows.zip",
                "https://example.invalid/LogiLite-Windows.zip",
    ...
    def test_same_volume_windows_stage_dir_is_adjacent_to_install_root(self):
        install_root = Path("C:/Users/example/LogiLite")
        ...
        self.assertEqual(stage_dir.name, ".LogiLite.update-v3.7.0-test-42")
    ...
    def test_launch_windows_helper_copies_executable_outside_install_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(helper, (helper_dir / "LogiLiteUpdateHelper" / "LogiLite.exe").resolve())
            ...
            self.assertEqual(Path(argv[0]).parent, (helper_dir / "LogiLiteUpdateHelper").resolve())
            self.assertEqual(argv[1:], ["--logilite-apply-update", str(plan_path)])
            self.assertEqual(cwd, str((helper_dir / "LogiLiteUpdateHelper").resolve()))
    ...
    def test_apply_windows_update_from_state_preserves_backup_and_installs_stage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (install / "LogiLite.exe").write_text("old", encoding="utf-8")
            (install / "_internal").mkdir()
            (staged / "LogiLite.exe").write_text("new", encoding="utf-8")
            (staged / "LogiLite.exe").chmod(0o755)
            ...
            self.assertEqual((install / "LogiLite.exe").read_text(encoding="utf-8"), "new")
            self.assertEqual((backup / "LogiLite.exe").read_text(encoding="utf-8"), "old")
    ...
    def test_apply_windows_update_from_state_restores_backup_after_transient_rollback_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            staged = stage_parent / "LogiLite"
            backup = root / "LogiLite.backup-123"
            marker = root / "last-update-result.txt"
            state = root / "pending-update.json"
            install.mkdir()
            staged.mkdir(parents=True)
            ...
            self.assertEqual((install / "LogiLite.exe").read_text(encoding="utf-8"), "old")
    ...
    def test_apply_windows_update_from_state_rejects_mismatched_staged_root_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan = WindowsUpdatePlan(
                ...
                install_root=str(install),
                staged_root=str(staged),
                backup_root=str(root / "LogiLite.backup-123"),
                result_marker=str(marker),
            )
            write_windows_update_plan(plan, state)
    ...
    def test_cleanup_stale_update_state_removes_pending_staging_and_helper(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            install = root / "install" / "LogiLite"
            install.mkdir(parents=True)
            stage_parent = same_volume_windows_stage_dir(install, "v3.7.0", pid=99)
            staged_root = stage_parent / "LogiLite"
            staged_root.mkdir(parents=True)
            (staged_root / "LogiLite.exe").write_text("new", encoding="utf-8")
            plan = WindowsUpdatePlan(
                current_pid=0,
                install_root=str(install),
                staged_root=str(staged_root),
                backup_root=str(root / "install" / "LogiLite.backup"),
                result_marker=str(root / "last-update-result.txt"),
            )
