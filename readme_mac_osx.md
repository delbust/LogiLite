...
LogiLite now supports macOS alongside Windows. This document covers macOS-specific setup and known differences.
...
LogiLite uses a **CGEventTap** to intercept and suppress mouse button events. macOS requires Accessibility permission for this:
...
5. Restart LogiLite if it was already running

If Accessibility is not granted, LogiLite will print:
```
...
| Config location | `%APPDATA%\LogiLite` | `~/Library/Application Support/LogiLite` |
...
- LogiLite also exposes macOS-specific actions such as **Mission Control**, **App Expose**, **Previous Desktop**, **Next Desktop**, **Show Desktop**, and **Launchpad**

### HID Access

On macOS, the HID gesture listener uses non-exclusive access (`hid_darwin_set_open_exclusive(0)`)
so the mouse continues to function normally while LogiLite reads HID++ reports.

### Trackpad and Magic Mouse Scroll

LogiLite ignores trackpad and Magic Mouse continuous scroll events by default so two-finger gestures and macOS natural scrolling keep working normally while mouse wheel mappings stay active.

You can change this in **Point & Scroll → Scroll Direction → Ignore trackpad**. Leave it enabled for built-in trackpads and most Logitech mouse setups. Disable it only if you intentionally want LogiLite to handle Magic Mouse or trackpad scroll events.
...
- The build flow uses the committed `images/AppIcon.icns` when present; otherwise the script generates an `.icns` icon from `images/logo_icon.png`, then runs PyInstaller with `LogiLite-mac.spec`.
- Signing path depends on `LOGILITE_SIGN_IDENTITY`. Unset: the bundle is ad-hoc signed (`codesign --sign -`), which is fine for one-off builds but can rotate the code identity on every rebuild, so macOS Accessibility grants may reset. Set to a codesigning identity (list with `security find-identity -v -p codesigning`, SHA-1 form preferred): the script signs nested `.dylib` / `.so` / `.framework` files depth-first with `--options runtime`, then signs the outer bundle with `build_resources/LogiLite.entitlements`, then runs `codesign --verify --deep --strict` and aborts the build if it fails. Stable permission behavior depends on unchanged source, resolved Python interpreter, dependencies, architecture, signing identity, entitlements, and timestamp policy.
...
- The app can then be moved to `/Applications/LogiLite.app` and launched directly from Finder, Spotlight, or Dock.
- `pyinstaller LogiLite.spec` remains available as a simpler cross-platform build path, but the dedicated macOS script is the preferred bundle flow.
- Release builds publish `LogiLite-macOS.zip` for Apple Silicon and `LogiLite-macOS-intel.zip` for Intel Macs.
...
LogiLite can now manage **Start at login** from the app UI on macOS.

- The toggle writes a LaunchAgent plist to `~/Library/LaunchAgents/io.github.delbust.logilite.plist`
...
- If **Start minimized** is enabled in LogiLite, the app still launches tray-first after login because that preference is read from config at startup
- Turning **Start at login** back off removes that LaunchAgent plist again

## Accessibility for the Packaged App

If you switch from Terminal-based startup to `LogiLite.app`, re-grant Accessibility for the app bundle:
